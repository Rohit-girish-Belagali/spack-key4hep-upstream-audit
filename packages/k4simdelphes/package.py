...
class K4simdelphes(CMakePackage):
    ...
    version(
        "00-07-05",
        sha256="54887cb8192470846e9e083bfbb6ff92f98555a2067c7c405b6490fdee2fb9b5",
    )
    version(
        "00-07-04",
    ...
    variant(
        "framework",
        default=True,
        description="Build Gaudi framework integration.",
    )
    variant(
        "integration_tests",
        default=True,
    ...
    depends_on(
        "evtgen+pythia8+tauola+photos",
        when="+delphes_pythia_evtgen",
    )
    ...
    def cmake_args(self):
        args = [
            ...
            self.define_from_variant(
                "BUILD_PYTHIA_READER", "delphes_pythia"
            ),
            self.define_from_variant(
                "BUILD_HEPMC_READER", "delphes_hepmc"
            ),
            self.define_from_variant(
                "BUILD_EVTGEN_READER", "delphes_pythia_evtgen"
            ),
            self.define(
                "CMAKE_CXX_STANDARD",
